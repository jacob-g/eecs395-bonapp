...
def page_data(db : DBConnector, metadata : dict):
    return {"alert_subscriptions": db.alerts_for(metadata["login_state"].user), "all_menu_items": db.all_menu_items((request.args["search"] + "%") if "search" in request.args else "%")}